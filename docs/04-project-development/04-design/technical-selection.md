...
- 这是目前最适合“滑块 + 图形点选”训练样本生成的开源底座。
- 它尤其适合做内部数据生成器，而不是必须上线成生产级公共服务。
