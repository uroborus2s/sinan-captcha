...
1. 严格按 `TASK-AT-001` 到 `TASK-AT-010` 顺序推进。
2. 每个任务都要产出可检查物，不接受“先写点代码再说”。
3. 前一任务未通过验收，后一任务不得启动。
4. 如果发现需要修改 study 契约、agent JSON 契约或 group 指标，必须回到对应任务重开。
...
| TASK-AT-010 | 实施前总验收 | 项目维护者 | 001-009 全部产物 | 准入结论、风险清单 | 允许进入实现 | 0.5 天 |
...
| 主执行角色 | 项目维护者 |
| 协作角色 | 所有前序角色 |
| 前置条件 | TASK-AT-001 到 TASK-AT-009 全部通过 |
| 主要输入 | 全部设计与任务产物 |
| 操作步骤 | 1. 逐项检查 001-009 是否有明确产物。<br>2. 检查 study 契约、command/skill 契约、group 策略是否冻结。<br>3. 形成准入结论：允许进入实现或打回补文档。 |
| 输出产物 | 实施准入结论、剩余风险清单、补项清单 |
| 验收标准 | 实现者不再需要追问“study 文件长什么样”“AI 到底能做什么”“group1/group2 怎么判好坏” |
| 阻断条件 | 任一关键前提仍未冻结 |
...
| 第 2 天下午 | TASK-AT-009 到 TASK-AT-010 | 优化边界冻结并形成实施准入结论 |
