...
- 关联需求：`REQ-001`、`REQ-004`、`REQ-008`、`REQ-010`、`REQ-015`、`NFR-004`、`NFR-007`
...
1. 严格按 `TASK-SOLVER-MIG-001` 到 `TASK-SOLVER-MIG-015` 推进。
...
| TASK-SOLVER-MIG-015 | 补齐全格式兼容矩阵与文档切换 | 测试负责人/文档维护者 | 输入适配实现、URL 安全策略 | 格式矩阵测试、用户文档更新、发布说明 | 文档与实现一致 | 0.5 天 |
...
| 预计工时 | 1 天 |
| 完成后进入 | TASK-SOLVER-MIG-013 |

## 16. TASK-SOLVER-MIG-013 执行表

| 字段 | 内容 |
|---|---|
| 任务目标 | 在不改变公开业务函数名的前提下，新增统一图片输入适配层，支持本地路径、`bytes`、base64 字符串与 `data:` URI |
| 主执行角色 | Python 实现者 |
| 协作角色 | 架构负责人、测试负责人 |
| 前置条件 | TASK-SOLVER-MIG-012 已通过 |
| 主要输入 | 公开 API 合同、异常合同、现有 `sinanz_image_io` |
| 操作步骤 | 1. 引入输入判别与统一解码入口。<br>2. 为 `bytes`、base64、`data:` URI 建立解码流程与临时资源管理策略。<br>3. 统一失败路径为结构化 `SolverInputError`。 |
| 输出产物 | 输入适配模块、错误码约束、类型回归测试 |
| 验收标准 | 对同一图片，路径输入和非路径输入得到同构业务结果；输入失败都有稳定错误类型 |
| 阻断条件 | 新增输入类型导致公开结果字段变化，或异常语义漂移 |
| 失败处理 | 回滚到仅路径模式并补充适配层单测后重试 |
| 预计工时 | 1 天 |
| 完成后进入 | TASK-SOLVER-MIG-014 |

## 17. TASK-SOLVER-MIG-014 执行表

| 字段 | 内容 |
|---|---|
| 任务目标 | 在输入适配层中新增 `http(s)` URL 图片输入，并冻结安全边界与可观测性 |
| 主执行角色 | Python 实现者 / 安全负责人 |
| 协作角色 | 测试负责人、发布维护者 |
| 前置条件 | TASK-SOLVER-MIG-013 已通过 |
| 主要输入 | 输入适配模块、运行时网络策略 |
| 操作步骤 | 1. 增加 URL 拉取器，限制协议为 `http/https`。<br>2. 增加文件大小上限、请求超时、重定向上限与可配置开关。<br>3. 增加下载与解码审计日志（不泄露敏感内容）。 |
| 输出产物 | URL 输入能力、安全策略配置、失败场景测试 |
| 验收标准 | URL 输入可用且受限；非法协议、超限、超时与坏图都能稳定阻断并返回结构化错误 |
| 阻断条件 | 出现无限下载、协议绕过或异常吞没 |
| 失败处理 | 先锁死 URL 输入功能开关并补安全门，再恢复 |
| 预计工时 | 1 天 |
| 完成后进入 | TASK-SOLVER-MIG-015 |

## 18. TASK-SOLVER-MIG-015 执行表

| 字段 | 内容 |
|---|---|
| 任务目标 | 建立 solver 输入兼容回归矩阵并完成用户文档切换，确保“实现、测试、文档”三位一体收口 |
| 主执行角色 | 测试负责人 / 文档维护者 |
| 协作角色 | Python 实现者、项目维护者 |
| 前置条件 | TASK-SOLVER-MIG-014 已通过 |
| 主要输入 | 输入适配实现、URL 安全策略、现有用户指南 |
| 操作步骤 | 1. 增加输入形态矩阵测试（Path/bytes/base64/data URI/URL）。<br>2. 增加格式兼容矩阵测试（JPEG/PNG/WebP/BMP/GIF/TIFF）。<br>3. 更新用户指南与函数参考，删除“仅本地路径”旧表述。 |
| 输出产物 | 回归测试集、文档更新、发布说明增量 |
| 验收标准 | CI 中输入矩阵稳定通过；用户文档与真实运行时行为一致 |
| 阻断条件 | 文档宣称能力与运行时行为不一致 |
| 失败处理 | 先回滚文档宣称到当前能力，再修补实现和测试 |
| 预计工时 | 0.5 天 |
| 完成后进入 | 正式代码迁移实施 |
