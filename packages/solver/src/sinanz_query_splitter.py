"""Rule-based query splitter for standalone group1 solving."""
...
def split_query_icons(query_image_path: Path) -> list[QuerySplitTarget]:
...
def _load_pillow_image():
    try:
        ...
        raise SolverRuntimeError("group1 query splitter requires Pillow.") from exc
    return Image


def _build_foreground_mask(image: object) -> list[list[bool]]:
...
def _border_pixels(pixels: object, *, width: int, height: int) -> list[tuple[int, int, int, int]]:
