...
GROUP1_CLASS_NAMES_STATUS = "instance_matching_v1"
...
def _write_group1_ready_metadata(metadata_dir: Path) -> None:
    ...
    (metadata_dir / "class_names.json").write_text(
        json.dumps(
            {
                "status": GROUP1_CLASS_NAMES_STATUS,
                "group1_class_names": [],
            },
            ensure_ascii=False,
            indent=2,
...
def _load_group1_embedder_export_backend() -> tuple[Any, Any]:
    ...
    try:
        embedder_module = importlib.import_module("core.train.group1.embedder")
    except ModuleNotFoundError as exc:  # pragma: no cover - environment dependent
        raise ValueError(
            "group1 icon embedder ONNX export requires `core.train.group1.embedder` dependencies."
        ) from exc
