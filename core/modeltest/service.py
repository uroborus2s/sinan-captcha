...
from core.train.group1.dataset import load_group1_dataset_config, load_group1_rows
...
def _summarize_verdict(task: str, metrics: dict[str, float | None]) -> tuple[str, str]:
    if task == "group1":
        ...
        return "这轮模型还在起步阶段", "先回头检查 query_targets/scene_targets 契约、类别表和两套子模型训练是否一致。"
